...
export default function ProductList({ onCheckout, subCategoryId, onBack, cart, onUpdateQuantity }: { onCheckout: () => void, subCategoryId: string | null, onBack?: () => void, cart: Record<string, number>, onUpdateQuantity: (productId: string, delta: number) => void }) {
  ...
  const cartItemsCount: number = (Object.values(cart) as number[]).reduce((sum: number, qty: number) => sum + qty, 0);
  const cartTotal: number = Object.entries(cart).reduce((sum: number, [id, qty]) => {
        ...
        {filteredProducts.map((product) => {
          ...
          return (
            <motion.div 
              key={product.id} 
              whileTap={{ scale: 0.98 }}
              onClick={() => quantity === 0 && onUpdateQuantity(product.id, 1)}
              className="bg-surface-container-lowest rounded-3xl p-3 flex flex-col gap-3 shadow-sm group text-left border border-transparent hover:border-primary/20 transition-all relative cursor-pointer"
            >
              ...
              <div className="px-1">
                ...
                <div className="mt-3 flex items-center justify-center">
                  <div className="flex items-center gap-4 bg-surface-container-low rounded-2xl p-1 shadow-inner">
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className={cn(
                        ...
                        quantity > 0 ? "text-primary hover:bg-primary/10" : "text-on-surface-variant/20 cursor-not-allowed"
                      )}
                      disabled={quantity === 0}
                    >
                      <Minus className="w-4 h-4" />
                    </button>
                    <span className="w-6 text-center text-sm font-black font-headline text-on-surface">{quantity}</span>
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        onUpdateQuantity(product.id, 1);
                      }}
                      className="w-8 h-8 rounded-xl primary-gradient flex items-center justify-center text-white shadow-sm active:scale-90 transition-all"
                    >
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  ...
                  <p className="font-headline font-extrabold text-lg leading-none">{cartItemsCount} Products Selected</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <p className="text-xl font-black font-headline">${cartTotal.toFixed(2)}</p>
